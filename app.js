...
const SHEETS = {
	...
	positions: {
		SHEET: "Positions",
		DOC: DATABASE_DOC,
		COLS: ["position", "type", "email", "key", "responsibilities", "filled"],
	},
	merch: {
		SHEET: "Merch",
		DOC: DATABASE_DOC,
...
function makeOpenings() {
	...
	for (let i = 0; i < data.positions.length; i++) {
		if (
			...
			true ||
			getCell("positions", i, "filled") == true
		) {
			continue;
		} // skip blank entries
		posCount++;
...
function makeSponsors() {
	document.getElementById("package").setAttribute("src", driveUrlToPreview("https://drive.google.com/file/d/1JFjuADqjNiTIVkD6PpZ6IxTFT3zMWaru/view?usp=drive_link"));

	let key = "";
	for (let i = 0; i < data.contacts.length; i++) {
		if (getCell("contacts", i, "option") == "Sponsorship") {
			let searchEmail = getCell("contacts", i, "override") != null ? getCell("contacts", i, "override") : getCell("contacts", i, "email") != null ? getCell("contacts", i, "email") : getCell("contacts", 0, "email");
			for (let j = 0; j < data.positions.length; j++) {
				if (getCell("positions", j, "email") == searchEmail) {
					key = getCell("positions", j, "key") != null ? getCell("positions", j, "key") : getCell("positions", 0, "key");
					break;
				}
			}
			break;
