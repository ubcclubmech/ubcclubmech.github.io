...
function makeLockers() {
  ...
  for (let i = 0; i < data.sets.length; i ++) {
    if (anyCellNull('sets', i, ['name', 'location', 'period', 'size', 'cost']) == true || getCell('sets', i, 'show') == false) { continue; } // skip blank entries
    ...
    for (let j = 0; j < data.lockers.length; j ++) {
      if (getCell('lockers', j, 'set') != getCell('sets', i, 'name') || getCell('lockers', j, 'number') == null) { continue; } // skip blank entries and lockers in other sets

      if ((lockerIdx + Number(getCell('sets', i, 'offset'))) % getCell('sets', i, 'count') == 0) {
        isOddCol = !isOddCol;
      }

      let rowIdx = (lockerIdx + Number(getCell('sets', i, 'offset'))) % getCell('sets', i, 'count');
      if (isWeave && !isOddCol) {
